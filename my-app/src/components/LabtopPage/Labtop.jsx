...
const STORAGE_KEY = 'laptops_cache';
const ETAG_KEY = 'laptops_etag';

class Labtop extends Component {
    ...
    loadFromCache() {
        try {
            if (LAPTOP_CACHE.data && LAPTOP_CACHE.timestamp &&
                (Date.now() - LAPTOP_CACHE.timestamp < LAPTOP_CACHE.expiry)) {
                this.setState({
                    products: LAPTOP_CACHE.data,
                    loading: false,
                    fromCache: true
                });
                return true;
            ...
            if (cached) {
                ...
                if (Date.now() - timestamp < LAPTOP_CACHE.expiry) {
                    LAPTOP_CACHE.data = data;
                    LAPTOP_CACHE.timestamp = timestamp;
                    this.setState({
                        products: data,
                        loading: false,
                        fromCache: true
            ...
            console.warn('Ошибка загрузки кэша ноутбуков:', error);
        }
        return false;
    }
    ...
    saveToCache(data) {
        try {
            LAPTOP_CACHE.data = data;
            LAPTOP_CACHE.timestamp = Date.now();
            localStorage.setItem(STORAGE_KEY, JSON.stringify({
                data: data.slice(0, 5),
                timestamp: Date.now()
            }));
            console.log('Ноутбуки: данные сохранены в кэш');
        } catch (error) {
            console.warn('Ошибка сохранения кэша ноутбуков:', error);
        }
    }

    componentDidMount() {
        console.log('Labtop component mounted');
        if (!this.state.fromCache) {
            this.fetchLaptopProducts();
        }
    }
    ...
    fetchLaptopProducts = async () => {
        if (this.abortController) this.abortController.abort();
        this.abortController = new AbortController();

        try {
            console.log('Загрузка ноутбуков');
            this.setState({ loading: true });

            const response = await fetch('http://localhost:5214/api/laptops', {
            ...
            console.log('Количество ноутбуков:', products.length);
    ...
    getLaptopType = (laptopName) => {
        if (!laptopName) return 'standard';
    ...
    getBrandName = (product) => {
        ...
        if (name.includes('xiaomi') || name.includes('redmibook')) return 'Xiaomi';
    ...
    handleClearCache = () => {
        ...
        LAPTOP_CACHE.data = null;
        LAPTOP_CACHE.timestamp = null;
        this.setState({ products: [], loading: true });
        this.fetchLaptopProducts();
        alert('Кэш ноутбуков очищен');
    };
    ...
    render() {
        ...
        console.log('Рендер Labtop, состояние:', {
            ...
            <div className="laptop-page">
                ...
                <main className="main-content">
                    <div className="container laptop-container">
                        ...
                        <h1 className="page-title">Ноутбуки</h1>

                        <div className="products-grid-container">
                            {loading && (
                                <div className="loading-spinner">
                                    <div className="spinner"></div>
                                    <p>Загрузка ноутбуков...</p>
                                </div>
                            )}
                                ...
                                <div className="products-grid">
                                    {displayProducts.length > 0 ? (
                                        displayProducts.map((product, index) => {
                                            const laptopType = this.getLaptopType(product.name);
                                            const laptopClass = `product-card ${laptopType}`;
                                            const brandName = this.getBrandName(product);
                                            const productName = product.name || `Ноутбук ${index + 1}`;
                                            const imageUrl = this.getProductImage(product);

                                            return (
                                                <div key={product.id || index} className={laptopClass} onClick={() => this.handleProductClick(product.id)} style={{ cursor: 'pointer' }}>
                                                    ...
                                                    <div className="product-image-container">
                                                        {imageUrl ? (
                                                            <img
                                                                src={imageUrl}
                                                                alt={product.name || `Ноутбук ${index + 1}`}
                                                                className={`product-image ${laptopType}`}
                                                                loading="lazy"
                                                                onError={(e) => {
                                                    ...
                                                    <div className="product-content">
                                                        ...
                                                        <div className="product-specs">
                                                            ...
                                                            {product.ram && (
                                                                <span className="spec">
                                                                    <span className="spec-label">ОЗУ:</span>
                                        ...
                                        <div className="no-products">
                                            <p>В базе данных нет ноутбуков</p>
                                            <div className="suggestions">
                                                <p>Что можно сделать:</p>
                                                <ul>
                                                    <li>Добавьте ноутбуки через админ-панель</li>
...
export default withNavigate(Labtop);