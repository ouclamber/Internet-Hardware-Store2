...
const STORAGE_KEY = 'phones_cache';
const ETAG_KEY = 'phones_etag';
...
class Phones extends Component {
    ...
    loadFromCache() {
        try {
            if (PHONE_CACHE.data && PHONE_CACHE.timestamp &&
                (Date.now() - PHONE_CACHE.timestamp < PHONE_CACHE.expiry)) {
                this.setState({ 
                    products: PHONE_CACHE.data, 
                    loading: false, 
                    fromCache: true 
                });
                return true;
            ...
            if (cached) {
                ...
                if (Date.now() - timestamp < PHONE_CACHE.expiry) {
                    PHONE_CACHE.data = data;
                    PHONE_CACHE.timestamp = timestamp;
                    this.setState({ 
                        products: data, 
                        loading: false, 
                        fromCache: true 
            ...
            console.warn('Ошибка загрузки кэша смартфонов:', error);
        }
        return false;
    }
    ...
    saveToCache(data) {
        try {
            PHONE_CACHE.data = data;
            PHONE_CACHE.timestamp = Date.now();
            localStorage.setItem(STORAGE_KEY, JSON.stringify({
                data: data.slice(0, 5),
                timestamp: Date.now()
            }));
            console.log('Смартфоны: данные сохранены в кэш');
        }
        catch (error) {
            console.warn("Ошибка сохранения кэша смартфонов:", error);
        }
    }

    componentDidMount() {
        console.log('Phones component mounted');
        if (!this.state.fromCache) {
            this.fetchPhoneProducts();
        }
    }
    ...
    fetchPhoneProducts = async () => {
        ...
        try {
            console.log('Загрузка смартфонов');
            this.setState({ loading: true });

            const response = await fetch('http://localhost:5214/api/smartphones', {
            ...
            console.log('Количество смартфонов:', products.length);
            ...
            console.error('Ошибка загрузки смартфонов:', error);
    ...
    getSmartphoneType = (phoneName) => {
        if (!phoneName) return 'standard';
    ...
    getBrandName = (product) => {
        ...
        if (name.includes('xperia') || name.includes('sony')) return 'Sony';
    ...
    handleClearCache = () => {
        ...
        PHONE_CACHE.data = null;
        PHONE_CACHE.timestamp = null;
        this.setState({ products: [], loading: true });
        this.fetchPhoneProducts();
        alert('Кэш смартфонов очищен');
    };
    ...
    render() {
        ...
        console.log('Рендер Phones, состояние:', { 
                                ...
                                <div className="products-grid">
                                    {displayProducts.length > 0 ? (
                                        displayProducts.map((product, index) => {
                                            const smartphoneType = this.getSmartphoneType(product.name);
                                            const smartphoneClass = `product-card ${smartphoneType}`;
                                            const brandName = this.getBrandName(product);
                                            const productName = product.name || `Смартфон ${index + 1}`;
                                            const imageUrl = this.getProductImage(product);

                                            return (
                                                <div key={product.id || index} className={smartphoneClass} onClick={() => this.handleProductClick(product.id)} style={{ cursor: 'pointer' }}>
                                                    ...
                                                    <div className="product-image-container">
                                                        {imageUrl ? (
                                                            <img 
                                                                src={imageUrl} 
                                                                alt={product.name || `Смартфон ${index + 1}`}
                                                                className={`product-image ${smartphoneType}`}
                                                                loading="lazy"
                                                                onError={(e) => {
                                        ...
                                        <div className="no-products">
                                            <p>В базе данных нет смартфонов</p>
                                            <div className="suggestions">
                                                <p>Что можно сделать:</p>
                                                <ul>
                                                    <li>Добавьте смартфоны через админ-панель</li>
...
export default withNavigate(Phones);